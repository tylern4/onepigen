subroutine daresbury(q2, win, ep, cscm, phcm, opt, sigma0)
    ...
    !     cscm=cos(theta_pi_cm)
    !     phcm=phi_pi_cm
    !     win=invariant hadronic mass, .ge. 1.205
    !     q2=Q squared
    !     ep=polarization, epsilon
    !     opt=1, calculate cross section as function of angles

    real a(4, 11), c(2, 11), d(3, 11), wdat(11), pi, sigma0
    integer opt

    data a/16.17, -0.57, -7.50, 1.87, 19.3, 1.68, -9.75, 0.62, &
            15.3, 2.72, -9.38, -1.64, 9.65, 0.35, -7.24, 0.19, &
            6.65, 0.93, -4.71, -0.57, 5.34, 1.16, -4.14, -0.98, &
            3.90, 1.16, -2.77, -0.95, &
            3.83, 0.51, -3.29, -0.29, 3.31, 0.21, -3.16, -0.34, &
            3.83, 0.52, -3.28, -0.68, 4.30, 0.83, -5.24, -2.73/

    data c/-8.00, -5.08, -10.37, 0.16, -6.37, 0.74, -5.32, -0.58, &
            -2.39, -0.23, -1.44, 0.34, -1.18, 0.6, -1.03, -0.22, &
            -1.23, -0.57, -1.28, -1.88, -2.44, -2.23/

    data d/-1.14, -2.72, 2.87, -0.36, -4.43, -0.48, -0.03, -2.09, 0.26, &
            0.87, -1.06, -1.21, 0.02, -0.37, -0.21, -0.18, -0.2, -0.7, &
            0.01, -0.51, -1.18, -0.43, -0.17, 0.07, -0.25, 0.18, -0.26, &
            -0.57, 0.2, -0.37, -0.38, -0.3, -1.22/
