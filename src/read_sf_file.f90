...
! READ_DATA_FILE
!
! Reads data file for use in interpolation
!
!EV Both these routines operate in double precision arithmetic
!   to suppress round off errors in the interpolation procedure.
!   But INTERP returns single precision results.
...
subroutine read_sf_file(rfile, iunit)
    ...
    !      write(6,*) 'Input method of spline fitting (1=cubic, 2=linear)'
    !      read(5,*) method_spline
    method_spline = 2
    ...
    ! Determine minimum/maximum of variables
    ...
    ! Now construct natural cubic splines in the 2nd dimension (VAR2) and
    ! calculate the 2nd derivatives wrt. VAR2 (2nd entry of each D2SF) of
    ! each structure function (SF). This is done only once, so do it here.
    ...
    elseif(method_spline.eq.2) then
        !****************************************************************
        ! the preparation for linear spline fitting
        !
        do jvar1 = 1, nvar1         ! Q2
    ...
    ! 1000 STOP ' Error reading the structure function input file.'
