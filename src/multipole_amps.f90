...
! multipole_amps
!
! Fills multipole amplitudes using results from interpolation
!------------------------------------------------------------------------------
subroutine multipole_amps
    ...
    !	nu_cm * S = qv_mag_cm * L
    !
    ! We have to multiply nu_cm / qv_mag_cm because we are following Tiator's notation
    ! which uses L-multipole rather than S-multipole.
    ! Note that Sato-Lee uses S-multipole.
    !
    factor = 0.141383d0 * nu_cm / qv_mag_cm
    !        factor = nu_cm / qv_mag_cm
    !        factor = 0.141383d0
    ...
    !
    !        print *, 'multipoles ------'
    !        print *, (sp(i),sm(i),ep(i),em(i),mp(i),mm(i),i=0,5)
    return
end